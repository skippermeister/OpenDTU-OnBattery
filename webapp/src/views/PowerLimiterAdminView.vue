<template>
    <BasePage :title="'Dynamic Power limiter Settings'" :isLoading="dataLoading">
        ...
        <form @submit="savePowerLimiterConfig" v-if="!configAlert">
            ...
            <CardElement
                :text="$t('powerlimiteradmin.SurplusPower')"
                textVariant="text-bg-primary"
                add-space
                v-if="'surplus_power_enabled' in powerLimiterConfigList && canUseSurplusPower"
            >
                <div class="alert alert-secondary" role="alert" v-html="$t('powerlimiteradmin.SurplusPowerInfo')"></div>
...
export default defineComponent({
    ...
    computed: {
        ...
        canUseSurplusPower() {
            ...
            const canUse = this.isEnabled && meta.charge_controller_enabled && this.governingBatteryPoweredInverters;
            return canUse;
        },
