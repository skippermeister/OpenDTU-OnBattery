...
static constexpr char TAG[] = "[PowerMeterUdpSmaHomeManager]";
...
uint8_t* PowerMeterUdpSmaHomeManager::decodeGroup(uint8_t* offset, uint16_t grouplen)
{
    ...
    while (offset < endOfGroup) {
        ...
        MessageOutput.printf("%s Skipped unknown measurement: %d %d %d %d\r\n", TAG,
...
void PowerMeterUdpSmaHomeManager::loop()
{
    ...
    if (buffer[0] != 'S' || buffer[1] != 'M' || buffer[2] != 'A') {
        MessageOutput.printf("%s Not an SMA packet?\r\n", TAG);
        return;
    }
    ...
    do {
        ...
        MessageOutput.printf("%s Unhandled group 0x%04x with length %d\r\n", TAG,
                grouptag, grouplen);
        offset += grouplen;
