            ...
            if (checksum == it[12]) {
                ...
                switch (it[2]) {
                    case Command::REQUEST_RATED_CAPACITY_CELL_VOLTAGE:
                        ...
                        if (Battery._verboseLogging) MessageOutput.printf("%s rated capacity: %.3fAh,  rated cell voltage: %.3fV\r\n", TAG, _stats->ratedCapacity, _stats->_ratedCellVoltage);
                        break;
                    ...
                    case Command::REQUEST_STATUS:
                        ...
                        if (Battery._verboseLogging) {
                            char dIO[9];
                            dIO[8] = 0;
                            for (int i=0; i<8; i++) dIO[i] = (_stats->_dIO & (0x80 >> i)) ? '1':'0';
                    ...
                    case Command::REQUEST_TEMPERATURE:
                        ...
                        if (it[4] > 0 && it[4] < 4) {
                            for (uint8_t i=0; i<7; i++) {
                                int idx = (it[4]-1)*3+i;
                                if (idx >= _stats->_tempsNumber) break;
                    ...
                    case Command::REQUEST_CELL_VOLTAGE:
                        ...
                        if (it[4] > 0 && it[4] < 9) {
                            for (uint8_t i=0; i<3; i++) {
                                int idx = (it[4]-1)*3+i;
                                if (idx >= _stats->_cellsNumber) break;
                    ...
                    case Command::REQUEST_CELL_BALANCE_STATES:
                        ...
                        for (int i=0; i<6; i++) _stats->_cellBalance[i] = it[4+i];
                        ...
                        if (Battery._verboseLogging) {
                            ...
                            for(uint8_t j=0; j<3; j++) {
                                for (uint8_t i=0; i<8; i++) cellBalance[j*(8+1)+i] = (_stats->_cellBalance[j] & (0x80>>i))?'1':'0';
                                cellBalance[j*(8+1)+8] = ' ';
                            }
                            cellBalance[26] = 0;
