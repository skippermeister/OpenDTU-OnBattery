...
bool SpiManager::claim_bus(spi_host_device_t& host_device)
{
    for (int i = SPI_MANAGER_NUM_BUSES - 1; i >= 0; --i) {
        ...
        printf("Successfully claimed host device: %d\r\n", host_device);
        return true;
    }

    printf("ERROR: Couldn't claim host device: %d\r\n", host_device);
...
std::optional<uint8_t> SpiManager::claim_bus_arduino()
{
    ...
    if (!claim_bus(host_device))
        return std::nullopt;
    auto spi_bus = to_arduino(host_device);
    if (spi_bus)
        printf("Successfully claimed host device: %d SPI bus %d\r\n", host_device, *spi_bus);
    else
        printf("ERROR: Couldn't claim host device: %d\r\n", host_device);
...
std::shared_ptr<SpiBus> SpiManager::get_shared_bus(const std::string& bus_id)
{
    ...
    for (int i = 0; i < SPI_MANAGER_NUM_BUSES; ++i) {
        ...
        if (shared_buses[i]->get_id() == bus_id) {
            printf("using existing shared bus '%s'\r\n", bus_id.c_str());
