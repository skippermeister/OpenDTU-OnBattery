...
uint8_t MCP2515Class::initCANBuffers(void)
{
    ...
    {
        const bool extd = (i == 1);
        int rc = setFilter(filters[i], extd, 0);
        if (rc != CAN_OK)
        {
            return rc;
        }
...
uint8_t MCP2515Class::reset(const int8_t canIDMode, const CAN_SPEED_t canSpeed, const MCP2515_CLOCK_t canClock)
{
    ...
    if (setCANCTRL_Mode(MODE_CONFIG) != CAN_OK)
    {
    ...
    case (MCP_ANY):
        ...
        /*
                 case (MCP_STD):
                    ...
                    spi_modifyRegister(MCP_RXB0CTRL,
                            RXBnCTRL_RXM_MASK | RXB0CTRL_BUKT,
                            RXBnCTRL_RXM_STD | RXB0CTRL_BUKT);
                    spi_modifyRegister(MCP_RXB1CTRL, RXBnCTRL_RXM_MASK, RXBnCTRL_RXM_STD);
                    break;

                 case (MCP_EXT):
                    spi_modifyRegister(MCP_RXB0CTRL,
                            RXBnCTRL_RXM_MASK | RXB0CTRL_BUKT,
                            RXBnCTRL_RXM_EXT | RXB0CTRL_BUKT);
                    spi_modifyRegister(MCP_RXB1CTRL, RXBnCTRL_RXM_MASK, RXBnCTRL_RXM_EXT);
                    break;
        */
    case (MCP_STDEXT):
        spi_modifyRegister(MCP_RXB0CTRL,
        ...
        spi_modifyRegister(MCP_RXB1CTRL, RXBnCTRL_RXM_MASK, RXBnCTRL_RXM_STDEXT);
        break;
...
*********************************************************************************************************/
uint8_t MCP2515Class::sleep(void)
{
    if (getMode() != MCP_SLEEP)
        return setCANCTRL_Mode(MCP_SLEEP);
    else
        return CAN_OK;
}
...
*********************************************************************************************************/
uint8_t MCP2515Class::wake(void)
{
    uint8_t currMode = getMode();
    if (currMode != mcpMode)
        return setCANCTRL_Mode(mcpMode);
    else
        return CAN_OK;
}
...
*********************************************************************************************************/
uint8_t MCP2515Class::setMode(const CANCTRL_REQOP_MODE_t opMode)
{
    if (opMode != MCP_SLEEP) // if going to sleep, the value stored in opMode is not changed so that we can return to it later
        mcpMode = opMode;
...
*********************************************************************************************************/
uint8_t MCP2515Class::getMode()
{
    return spi_readRegister(MCP_CANSTAT) & MODE_MASK;
}
...
*********************************************************************************************************/
uint8_t MCP2515Class::setCANCTRL_Mode(const CANCTRL_REQOP_MODE_t mode)
{
    ...
    if ((spi_readRegister(MCP_CANSTAT) & MODE_MASK) == MCP_SLEEP && mode != MCP_SLEEP)
    {
        ...
        uint8_t wakeIntEnabled = (spi_readRegister(MCP_CANINTE) & CANINTF_WAKIF);
        if (!wakeIntEnabled)
...
uint8_t MCP2515Class::requestNewMode(const uint8_t newmode)
{
    ...
    while (1)
    {
        ...
        uint8_t statReg = spi_readRegister(MCP_CANSTAT);
...
uint8_t MCP2515Class::setBitrate(const CAN_SPEED_t canSpeed, MCP2515_CLOCK_t mcp2515Clock)
{
    ...
    set = 1;
    switch (mcp2515Clock & MCP_CLOCK_SELECT)
    {
    case (MCP_8MHZ):
        switch (canSpeed)
        {
    ...
    if (mcp2515Clock & MCP_CLKOUT_ENABLE)
    {
        cfg3 &= (~SOF_ENABLE);
    }
...
uint8_t MCP2515Class::setClkOut(const CAN_CLKOUT_t divisor)
{
    if (divisor == CLKOUT_DISABLE)
    {
        ...
        spi_modifyRegister(MCP_CNF3, SOF_ENABLE, SOF_ENABLE);
        return CAN_OK;
    }
    ...
    spi_modifyRegister(MCP_CNF3, SOF_ENABLE, 0x00);
    return CAN_OK;
}

void MCP2515Class::prepareId(uint8_t *buffer, const bool extd, const uint32_t id)
{
...
uint8_t MCP2515Class::setFilterMask(const MASK_t mask, const bool extd, const uint32_t ulData)
{
    uint8_t res = setCANCTRL_Mode(MODE_CONFIG);
    if (res != CAN_OK)
    {
        return res;
    }
    ...
    uint8_t tbufdata[4];
    prepareId(tbufdata, extd, ulData);
    ...
    res = setCANCTRL_Mode(mcpMode);

    return res;
}

uint8_t MCP2515Class::setFilter(const RXF_t num, const bool extd, const uint32_t ulData)
{
    uint8_t res = setCANCTRL_Mode(MODE_CONFIG);
    if (res != CAN_OK)
    {
...
uint8_t MCP2515Class::sendMessage(const TXBn_t txbn, can_message_t *tx_message)
{
    ...
    bool extd = tx_message->extd;
    bool rtr = tx_message->rtr;
    uint32_t id = (tx_message->identifier & (extd ? CAN_EFF_MASK : CAN_SFF_MASK));

    prepareId(data, extd, id);
...
uint8_t MCP2515Class::readMessage(const RXBn_t rxbn, can_message_t *rx_message)
{
    ...
    rx_message->data_length_code = (tbufdata[MCP_DLC] & DLC_MASK);
    if (rx_message->data_length_code > CAN_MAX_DLC)
    {
        rx_message->dlc_non_comp = 1;
        rx_message->data_length_code = CAN_MAX_DLC;
        rc = CAN_FAILRX_DLC;
    }
    ...
    spi_readRegisters(rxb->DATA, rx_message->data, rx_message->data_length_code);
